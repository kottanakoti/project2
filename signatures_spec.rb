...
describe "apitest process", type: :feature do
    scenario "API test" do 
        ...
        puts signature_data
        role_2 = post_signature_group_in_signatures(deal_id, signature_data, auth_cookie)
        find("[data-test-id='nav-item-signatures']").click
    end
  end
  ...
  class CreateSignatureGroup 
    ...
    def initialize(options = {})
      @signature_group = options.fetch(:signature_group, "signature group #{Time.now.to_i}")
    end
